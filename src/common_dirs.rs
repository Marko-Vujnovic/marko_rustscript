...
pub fn get_app_folder() -> std::path::PathBuf { let mut dir = std::path::PathBuf::new(); let username = os_username(); dir.push(format!("/home/{username}")); dir.join(&program_info.name) }
